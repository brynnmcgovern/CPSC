	...
	while(bUserWishesToContinue == true)
	{
		...
		else
		{
			...
			if (bUapAbruptManeuvers)
			{
				...
				if ((bUapDetectedByRadar == true) && (bUapDetectedByIR == true))
				{
					vecReason.push_back(strReasonUapDetectedByRadar);
					vecReason.push_back(strReasonDetectedByIR);/* vecReason.??; */
					/* vecReason.??; */
		...
		*/
		...
		else if((cUserResponse != 'Y') || (cUserResponse != 'y')){
